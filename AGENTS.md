...
## kernel-module-glowforge repository rules

`glowforge.ko` is the kernel driver for the factory control board: the
SDMA and EPIT pulse engine, the laser latch, the safety-chain readbacks, the
PIC analog and digital I/O, the thermal outputs, the head peripherals, and
the button and lid LEDs.

Two site pages are this repository's contract, and it carries no separate
document of its own:
<https://docs.forgefirm.org/technical/forgefirm/kernel-module/> and
...
`interlock_test` and `backtrack_test` cover the kernel-independent parts,
built with the host compiler against `src/`. CI cross-builds the module
against the image kernel with the BSP overlay and patches applied, under
`KCFLAGS=-Werror`.

**Do not build the module by hand for the board.** The recipe builds it
against the kernel of the image, and the module ships in that image.

### Rules specific to this repository

- **A `.ko` change is validated on the image that ships it**, never
  hot-swapped onto a board that is about to be reflashed. Kernel and BSP
  changes ride one image flash, batched.
- **A module hot-swap loads only if the module was built against the flashed
  kernel's patch state.** Any edit under the kernel recipe's overlay
  re-stamps the kernel version, and reverting the edit does not bring the
  stamp back.
- **The target is uniprocessor**, and the locking is written for it: the
  state spinlock serializes the main path against the SDMA and hrtimer
  callbacks. A build for a multiprocessor part needs that locking reviewed
  first, in particular every callback that touches driver state without the
  lock.
- **The fail-safe paths are the point of this driver.** The panic handler,
  the dead man's switch, the FIRE backstop and the interlock latch drive are
  what stand between a dead kernel and a machine that keeps playing its ring.
  Change one only with a test.
- The SDMA script is assembled at build time from `asm/sdma.asm` by a
  third-party assembler under `tools/`, whose headers stay unchanged.

### CI coupling that sets the push order

CI cross-builds against the BSP from `meta-openglow`. Push `meta-openglow`
before a module change that needs a BSP change.
